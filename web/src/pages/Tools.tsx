    ...
    <div className="space-y-6">
      ...
      {toolList?.length === 0 && (
        <div className="bg-slate-900 rounded-xl border border-slate-800 p-12 text-center">
          ...
          <p className="text-slate-500">No tools discovered. Connect a server to see its tools.</p>
        </div>
      )}

      <div className="space-y-6">
        {Object.entries(byServer || {}).map(([serverName, serverTools]) => (
          <div key={serverName} className="bg-slate-900 rounded-xl border border-slate-800">
            <div className="px-5 py-4 border-b border-slate-800 flex items-center gap-2">
              ...
              <span className="text-sm text-slate-500">· {serverTools?.length ?? 0} tools</span>
            </div>
                ...
                <div key={tool.name} className="px-5 py-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {tool.description && (
                        <div className="text-sm text-slate-500 mt-0.5">{tool.description}</div>
                      )}
                    </div>
                  </div>
                </div>
